...
void ClassViewer::printInstructionParameters(u1 *code, int &index)
{
  ...
  else if (code[index] == 0x13 or code[index] == 0x14 or (code[index] >= 0xb2 and code[index] <= 0xb8) or code[index] == 0xbb or code[index] == 0xbd or code[index] == 0xc0 or code[index] == 0xc1)
  {
    ...
    std::string constant_name = class_file->getNameFromConstantPoolEntry(class_file->constant_pool[cp_index - 1]);
  ...
  else if (code[index] == 0xc5)
  {
    ...
    std::cout << "#" << cp_index << " <" << class_file->getNameFromConstantPoolEntry(class_file->constant_pool[cp_index - 1]) << "> dim " << unsigned(code[index + 3]) << std::endl;
    index += 3;
  }
  else
  {
...
{
  std::string name = class_file->getNameFromConstantPoolEntry(class_info_entry);
...
{
  std::string name = class_file->getNameFromConstantPoolEntry(constant_pool_fieldref);
...
{
  std::string name = class_file->getNameFromConstantPoolEntry(methodref_info_entry);
...
{
  std::string name = class_file->getNameFromConstantPoolEntry(interfacemethodref_info_entry);
  std::string name_class_index = splitByToken(name, 0);
  std::string name_and_type_index = splitByToken(name, 1) + " : " + splitByToken(name, 2);
...
{
  std::string name = class_file->getNameFromConstantPoolEntry(string_info_entry);
...
{
  std::string name = class_file->getNameFromConstantPoolEntry(nameandtype_info_entry);
...
{
  std::string name = class_file->getNameFromConstantPoolEntry(methodhandle_info_entry);
...
{
  std::string name = class_file->getNameFromConstantPoolEntry(methodType_info_entry);
...
{
  std::string name = class_file->getNameFromConstantPoolEntry(invokeDynamic_info_entry);
